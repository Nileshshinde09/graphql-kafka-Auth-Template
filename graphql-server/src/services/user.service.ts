  ...
  public static createUser = asyncHandler(
    async (Payload: CreateUserPayload) => {
      const { user_name, first_name, last_name, email, avatar_url, password } =
        Payload;
      if (!user_name)
        return ApiError.GraphQLError(404, "Username not found!,It is required field.");
      if (!first_name)
        return ApiError.GraphQLError(404, "First Name not found!,It is required field.");
      if (!email)
        return ApiError.GraphQLError(404, "Email not found!,It is required field.");
      if (!password)
        return ApiError.GraphQLError(404, "Password not found!,It is required field.");
    }
  );
